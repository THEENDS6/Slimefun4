package cc.theends6.sfx.api.energy.runtime;

import cc.theends6.sfx.api.ui.SfxUiItems;
import cc.theends6.sfx.api.localization.SfxLocalizationView;
import cc.theends6.sfx.api.text.Text;
import java.util.List;
import java.util.Objects;
import net.kyori.adventure.text.Component;
import org.bukkit.Material;
import org.bukkit.inventory.ItemStack;

public record SfxEnergyComponentUiItem(Material material, String name, List<String> lore, String nameKey, String loreKey, boolean glint) {
    public SfxEnergyComponentUiItem {
        Objects.requireNonNull(material, "material");
        name = name == null ? " " : name;
        lore = lore == null ? List.of() : List.copyOf(lore);
        nameKey = blankToNull(nameKey);
        loreKey = blankToNull(loreKey);
    }

    public ItemStack toItemStack(SfxLocalizationView localization) {
        Component displayName = Text.renderFlexible(localizedName(localization));
        List<Component> lines = localizedLore(localization).stream().map(Text::renderFlexible).toList();
        ItemStack stack = SfxUiItems.named(material, displayName, lines);
        if (glint && stack.hasItemMeta()) {
            var meta = stack.getItemMeta();
            meta.setEnchantmentGlintOverride(Boolean.TRUE);
            stack.setItemMeta(meta);
        }
        return stack;
    }

    private String localizedName(SfxLocalizationView localization) {
        if (nameKey != null) {
            return localization.requiredText(nameKey);
        }
        if (name == null || name.isBlank()) {
            return " ";
        }
        throw new IllegalStateException("Energy UI item is missing name-key for non-blank name: " + name);
    }

    private List<String> localizedLore(SfxLocalizationView localization) {
        if (loreKey != null) {
            return localization.requiredList(loreKey);
        }
        if (lore == null || lore.isEmpty()) {
            return List.of();
        }
        throw new IllegalStateException("Energy UI item is missing lore-key for non-empty lore");
    }

    private static String blankToNull(String value) {
        return value == null || value.isBlank() ? null : value;
    }
}
