...
/**
 * Shared machine runtime registry and tick-status recorder used by all concrete SFX machine systems.
 *
 * <p>The runtime now acts as a capability pipeline: concrete services may still own their legacy
 * behavior, but every machine definition can declare capabilities, providers, policies and phase
 * effects. The engine executes registered phase hooks around all framework-wrapped ticks and records
 * a unified snapshot for debug, audit and future processor migration.</p>
 */
    ...
    private void notifyPhaseObservers(String machineId, SfxMachinePhase phase, UUID instanceId, Location location, SfxMachineStatus currentStatus, Map<String, Object> attributes) {
        ...
        for (SfxMachinePhaseObserver observer : phaseObservers) {
            try {
                ...
                // Observers are diagnostic only and must never affect machine behavior.
            }
        }
    }
