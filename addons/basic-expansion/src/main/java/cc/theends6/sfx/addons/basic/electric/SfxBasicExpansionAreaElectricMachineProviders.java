...
final class SfxBasicExpansionAreaElectricMachineProviders {
    ...
    private static volatile SfxRuntime runtime;
    ...
    static void clearCaches() {
        FLUID_POOL_CACHE.clear();
        FLUID_PUMP_SOURCE_CACHE.clear();
        runtime = null;
    }

    static void bindRuntime(SfxRuntime value) {
        runtime = Objects.requireNonNull(value, "runtime");
    }

    private static SfxRuntime runtime() {
        SfxRuntime value = runtime;
        if (value == null) {
            throw new IllegalStateException("Basic Expansion runtime is not bound");
        }
        return value;
    }
    ...
    private static boolean increaseCropAge(Block crop) {
        ...
        runtime().setBlockData("sf:area_machine", crop, ageable, false, "electric-area", "growth-accelerator");
        return true;
    }
    ...
    private static void resetHoney(Block block) {
        ...
        if (data instanceof Beehive hive) {
            ...
            runtime().setBlockData("sf:area_machine", block, hive, false, "electric-area", "produce-collector:hive-reset");
        }
    }
    ...
    private static void pushOutput(SfxItems items, SfxElectricMachineDefinition definition, SfxElectricMachineState state, SfxElectricStack output) {
        ...
        SfxElectricOutputs.insert(items, state, slot, output);
    }
    ...
    private static SfxElectricMachineTickResult completeFluidPump(SfxAreaMachineRules rules, JavaPlugin plugin, SfxItems items, SfxElectricMachineDefinition definition, SfxElectricMachineState state, Location location) {
        ...
        if (action.consumeSource()) {
            runtime().setBlockType(definition.id(), action.source(), Material.AIR, true, "electric-area", "consume-source");
        }
        state.resetProgress();
        return SfxElectricMachineTickResult.changed(SfxElectricMachineRenderStatus.WORKING, 0, true);
    }
